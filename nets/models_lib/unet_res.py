...
def encoder(x):
    ...
    main_path = res_block(main_path, [64, 64], [(2, 2), (1, 1)])  # 第三层([128,128]是两层卷积核的个数,2代表卷积核大小,1代表步长)
    to_decoder.append(main_path)

    main_path = res_block(main_path, [128, 128], [(2, 2), (1, 1)])  # 第四层
    to_decoder.append(main_path)

    main_path = res_block(main_path, [256, 256], [(2, 2), (1, 1)])  # 第四层
    to_decoder.append(main_path)
...
def unet_model(inputs):
    ...
    path = res_block(to_decoder[3], [512, 512], [(2, 2), (1, 1)])  # 第五层
