...
def encoder(x):
    ...
    skip = Conv2D(filters=256, kernel_size=(3, 3), padding='same', strides=(8, 8))(skip)
...
def decoder(x, from_encoder):
    ...
    main_path = UpSampling2D(size=(2, 2))(main_path)
    main_path = concatenate([main_path, from_encoder[0]], axis=3)  # 9层和第1层跳接
...
def unet_model(inputs):
    ...
    # 中间层  4层
    res_path = BatchNormalization()(to_decoder[2])
    res_path = Activation(activation='relu')(res_path)
    ...
    shortcut = Conv2D(filters=256, kernel_size=(1, 1), strides=(2, 2))(to_decoder[2])
    shortcut = BatchNormalization()(shortcut)
    ...
    # unet decoder
    path = decoder(path_all, from_encoder=to_decoder)  # 678层
